...
function reator (config) {
  var self = this
  if (!(self instanceof reator)) return new reator(config)
...
reator.prototype._initRemote = function (cb) {
  var self = this

  function onRoom (data) {
    self.roomID = data.room
    Interface.setRoom(self.roomID)
    window.history.pushState('reator', lg('history_item', {room: self.roomID}), '?room=' + self.roomID + (self.embed ? '&embed=true' : ''))
